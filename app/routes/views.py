...
async def add_task(request: Request):
    ...
    try:
        ...
        entry_data = VaultEntrySchema(**data)
...
async def delete_task(request: Request):
    ...
    async with AsyncSessionLocal() as session:
        stmt = delete(VaultEntry).where(
            VaultEntry.id == task_id,
            VaultEntry.user_id == current_user_id
        )
        await session.execute(stmt)
        await session.commit()
...
async def delete_all_passwords(request: Request):
    ...
    async with AsyncSessionLocal() as session:
        stmt = delete(VaultEntry).where(VaultEntry.user_id == current_user_id)
        result = await session.execute(stmt)
        await session.commit()
...
async def edit(request: Request):
    ...
    async with AsyncSessionLocal() as session:
        stmt = select(VaultEntry).where(VaultEntry.id == task_id, VaultEntry.user_id == current_user_id)
        result = await session.execute(stmt)
